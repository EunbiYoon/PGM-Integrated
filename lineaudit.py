...
axes[0,0] = fig.add_subplot(gs[0:3,0:5])
axes[0,1] = fig.add_subplot(gs[0:3,6:11])
axes[1,0] = fig.add_subplot(gs[3:6,0:5])
axes[1,1] = fig.add_subplot(gs[3:6,6:11])
axes[2,0] = fig.add_subplot(gs[6:10,0:12])
...
axes[1,0].set_title('Balance Weight Bolt Torque Result',x=0.5,y=0.7,fontsize=10)
...
axes[1,1].set_title('Door Latch Gap Result',x=0.5,y=0.7,fontsize=10)



######################################################### GMES table 결과 도출 #####################################################
col_colours=['#FBEEB0','#CED6FE','#CED6FE','#CED6FE','#CED6FE','#CED6FE','#C5EFE8','#C5EFE8','#FCD9E0']
data_table=axes[2,0].table(cellText=GMES_data.values,rowLabels=GMES_data.index,colLabels=GMES_data.columns, loc='center',colLoc='center',rowLoc='left',cellLoc='left',colColours=col_colours)
data_table.set_fontsize(9)
data_table.auto_set_column_width(col=list(range(len(GMES_data.columns))))

today=date.today()
today=today.strftime('%m/%d')
axes[2,0].set_title('Front Loader Line Audit Result ('+today+")",x=0.5, y=0.95,fontsize=10)
